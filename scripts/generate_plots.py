...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate scientific plots for the experiment"
    )
...
def throughput_batch_vs_stream(
    batch_summary: List[Dict[str, str]],
    stream_summary: List[Dict[str, str]],
    output_path: Path,
) -> None:
        ...
        rows.append(
            {
                ...
                "mode": "Batch",
                "throughput": to_float(
                    row.get("throughput_rows_per_sec_mean")
                ),
            }
        )
        ...
        rows.append(
            {
                ...
                "mode": "Stream",
                "throughput": to_float(
                    row.get("stream_throughput_rows_per_sec_mean")
                ),
            }
        )

    if not rows:
        print("[WARN] No data for throughput plot")
        return
...
def latency_boxplot(
    stream_microbatch_rows: List[Dict[str, str]],
    output_path: Path,
) -> None:
    ...
    rows = [
        {
            "scenario": row.get("scenario", "unknown"),
            "latency_s": to_float(row.get("avg_latency_s")),
        }
    ...
    if not rows:
        print("[WARN] No data for latency plot")
        return
...
def cpu_usage_comparison(
    container_summary: List[Dict[str, str]],
    output_path: Path,
) -> None:
    ...
    if not rows:
        print("[WARN] No data for CPU comparison plot")
        return
...
def scalability_workers(
    ...
    rows = [
        {
            "scenario": row.get("scenario", "unknown"),
            "workers": int(float(row.get("workers", 0) or 0)),
...
def combined_scalability_plot(
    ...
    if not rows:
        print("[WARN] No data for combined scalability plot")
        return
...
def main() -> None:
    ...
    batch_summary = read_csv_rows(
        summary_dir / "batch_summary.csv"
    )

    stream_summary = read_csv_rows(
    summary_dir / "stream_microbatch_summary.csv"
)

    container_summary = read_csv_rows(
        summary_dir / "container_summary.csv"
    )

    stream_microbatches = read_csv_rows(
        raw_dir / "stream_microbatches.csv"
    )

    throughput_batch_vs_stream(
        batch_summary,
        stream_summary,
        plots_dir / "throughput_batch_vs_stream.png",
    )

    latency_boxplot(
        stream_microbatches,
        plots_dir / "latency_boxplot.png",
    )

    cpu_usage_comparison(
        container_summary,
        plots_dir / "cpu_usage_comparison.png",
    )
    ...
    combined_scalability_plot(
        batch_summary,
        stream_summary,
        plots_dir / "scalability_workers.png",
    )

    print(f"[OK] Plots generated in: {plots_dir}")
